...
module.exports.showIndividualList = async(req, res)=>{
    ...
    let { id } = req.params;
    const list = await Listing.findById(id).populate({ path: "reviews", populate: { path:"author"}}).populate("owner");     //Listing.find({_id : id});  POPULATE is use to send array data also [one to many relation in mongodb]
        if(!list){
...
//  await Listing.findByIdAndUpdate(id, req.body.Listing); //2d time update if there a 
    ...
    ...req.body.Listing,     
    available: true  // new field added here
    });
